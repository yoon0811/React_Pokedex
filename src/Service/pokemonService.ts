...
interface PokemonDetailResoponseType {
  id: number,
  weight: number,
  height: number,
  name: string,
...
interface PokemonSpeciesResoponseType{
  color:{
    name:string
  },
  names: {
...
export const fetchPokemonDetail = async (name: string):Promise<PokemonDetailType> => {
  ...
  const response = await remote.get<PokemonDetailResoponseType>(pokemonDetailUrl)
  const speciseResponse= await remote.get<PokemonSpeciesResoponseType>(pokemonSpeciesUrl)
  const detail = response.data;
  const specise = speciseResponse.data;

  const koreanName = specise.names.find(item => item.language.name ==='ko')?.name ?? detail.name;
  
  return {
    id: detail.id,
    name: detail.name,
    color: specise.color.name,
